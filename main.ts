...
async function playGame() {
  ...
  while (continueProgram) {
    ...
    if (levels === "Exit To Game:") {
      ...
      console.log(
        chalk.green.bold.italic(`\tGAME CREATOR : => `) +
    ...
    const { opnSelect } = await inquirer.prompt([
      {
        ...
        message: `Select ${opponents[
          opponentIndex
        ].toUpperCase()} To Fight.`,
      },
    ]);
      ...
      while (continueMode) {
        ...
        if (number > 0) {
          player.decreaseHealth(
            opponentIndex === 0 ? 20 : opponentIndex === 1 ? 30 : 50,
          );
          console.log(
            chalk.green.bold.italic(
          ...
          opponent.decreaseHealth(
            opponentIndex === 0 ? 50 : opponentIndex === 1 ? 30 : 30,
          );
          console.log(
            chalk.green.bold.italic(
              `\n\t${player.name} Health is: ${player.health}`,
          ...
          const { health } = await inquirer.prompt([
            {
              ...
              message:
                "If you need Health CLICK On Option 1, Otherwise click on option 2 to Give Up .",
            },
          ]);
